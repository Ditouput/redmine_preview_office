      ...
      def self.included(base)
        ...
        base.class_eval do
		  ...
		  # Generates a thumbnail for the source image to target
			...
			print "#{target} is the target, and #{source} is the source"
			FileUtils.mkdir_p "/tmp/thumbnails/tmp"

			unless File.exists?(target)

			  Dir.mktmpdir(nil, "/tmp/thumbnails/tmp") do |dir|
			    if Redmine::Platform.mswin?
			      cmd = "cd #{dir}; export HOME=/tmp/thumbnails/tmp & #{shell_quote @REDMINE_PREVIEW_OFFICE_CONVERT_BIN} --headless --invisible --convert-to pdf #{shell_quote source} & move #{shell_quote File.basename(source, File.extname(source)) + ".pdf"} #{shell_quote target}"
			    else
				  cmd = "cd #{dir}; export HOME=/tmp/thumbnails/tmp & #{shell_quote @REDMINE_PREVIEW_OFFICE_CONVERT_BIN} --headless --invisible --convert-to pdf #{shell_quote source}; mv #{shell_quote File.basename(source, File.extname(source)) + ".pdf"} #{shell_quote target}"
                end
