        ...
        base.class_eval do
		  ...
		  def self.generate_preview_office(source, target )
			...
			unless File.exists?(target)
			  ...
			  directory = "/tmp/officeviewer"
			  unless File.exists?(directory)
				FileUtils.mkdir directory
			  end
			  			                
			  Dir.mktmpdir(nil, "/tmp/officeviewer") do |tmpdir|
			    if Redmine::Platform.mswin?
			      cmd = "cd #{directory} & #{shell_quote @REDMINE_PREVIEW_OFFICE_CONVERT_BIN} --convert-to pdf #{shell_quote source} & move #{shell_quote File.basename(source, File.extname(source)) + ".pdf"} #{shell_quote target}"
			    else
				  cmd = "cd #{directory}; #{shell_quote @REDMINE_PREVIEW_OFFICE_CONVERT_BIN} --convert-to pdf #{shell_quote source}; mv #{shell_quote File.basename(source, File.extname(source)) + ".pdf"} #{shell_quote target}"
                end
