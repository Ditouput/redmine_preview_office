        ...
        base.class_eval do
		  ...
		  # Generates a thumbnail for the source image to target
			def self.generate_preview_office(source, target)
				my_dir = Dir["/usr/src/redmine/tmp/thumbnails/*.pdf"]
				my_dir.each do |filename|
					name = File.basename('filename', '.pdf')
					dest_dir = "/tmp/#{name}"
					FileUtils.cp(filename, dest_dir)
				end
			print "#{target} is the target, and #{source} is the source, and the target has been copied to #{dest_dir}"
			...
			unless File.exists?(target)
			  ...
			  Dir.mktmpdir(nil, "/tmp") do |dir|
			    if Redmine::Platform.mswin?
			      cmd = "cd #{dir} & #{shell_quote @REDMINE_PREVIEW_OFFICE_CONVERT_BIN} --convert-to pdf #{shell_quote source} & move #{shell_quote File.basename(source, File.extname(source)) + ".pdf"} #{shell_quote target}"
			    else
				  cmd = "cd #{dir}; #{shell_quote @REDMINE_PREVIEW_OFFICE_CONVERT_BIN} --convert-to pdf #{shell_quote source}; mv #{shell_quote File.basename(source, File.extname(source)) + ".pdf"} #{shell_quote target}"
                end
