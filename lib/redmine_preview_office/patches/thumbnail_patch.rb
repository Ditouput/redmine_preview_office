      ...
      def self.included(base)
        ...
        base.class_eval do
		  ...
		  # Generates a thumbnail for the source image to target
			...
			dir = FileUtils.mkdir_p "/tmp/thumbnails/tmp"

			unless File.exists?(target)


			    if Redmine::Platform.mswin?
			      cmd = "cd #{dir}; export HOME=/tmp/thumbnails && #{shell_quote @REDMINE_PREVIEW_OFFICE_CONVERT_BIN} --headless --invisible --convert-to pdf #{shell_quote source} & move #{shell_quote File.basename(source, File.extname(source)) + ".pdf"} #{shell_quote target}"
			    else
				  cmd = "cd #{dir}; export HOME=/tmp/thumbnails && #{shell_quote @REDMINE_PREVIEW_OFFICE_CONVERT_BIN} --headless --invisible --convert-to pdf #{shell_quote source}; mv #{shell_quote File.basename(source, File.extname(source)) + ".pdf"} #{shell_quote target}"
					end
				unless system(cmd)
